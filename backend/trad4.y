...
b_variables:   '(' SETQ IDENTIF termino ')'     { printf ("variable %s\n", $3.code) ;  
						                        if ($4.code!=0) { printf ("%s %s !\n", $4.code, $3.code); } ; } //Si termino no es 0, asignamos valor  
	    ;
...
funcion:   IDENTIF { sprintf (funcion, "%s", $1.code); } 
	   '(' lista_parametros ')' '{' sentencias '}'  { printf("(defun %s(%s) %s)\n", $1.code, $4.code, $7.code);}
	  ;
...
main:        MAIN { sprintf (funcion, "%s", $1.code); } '(' ')' '{' sentencias '}'  { printf("(defun main() %s)\n", $6.code);}
	    ;
...
sentencia:    IDENTIF '=' expresion      { sprintf (temp, "(setf %s_%s %s)", funcion, $1.code, $3.code) ; 
                                           $$.code = gen_code (temp) ; }
            | IDENTIF '[' expresion ']' '=' expresion { sprintf (temp, "(setf %s_(aref %s %s) %s)", funcion, $1.code, $3.code, $6.code) ; 
                                           $$.code = gen_code (temp) ; }
	    ...
	    | IDENTIF '(' ')'     	 { sprintf (temp, "(%s)", $1.code) ;
                                	   $$.code = gen_code (temp) ; } 
            | RETURN expresion           { sprintf(temp, "return-from %s %s", funcion, $2.code);
                            		   $$.code = gen_code(temp);}
            ;
...
expresion:      termino                  { $$ = $1 ; }
            ...
            |   IDENTIF '(' ')'     	 { sprintf (temp, "(%s)", $1.code) ;
                                	   $$.code = gen_code (temp) ; } 
            |   IDENTIF '[' expresion ']'   { sprintf (temp, "(setf %s_%s %s)", funcion, $1.code, $3.code) ; 
                                           $$.code = gen_code (temp) ; }
            ;
